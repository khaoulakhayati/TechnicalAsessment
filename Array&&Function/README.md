...
1. In your console, copy the following function and verify
   ...
   square(10) + 2; // 100+2=102
   square(100) + square(77);//10000 + 5929 = 15929
   square(8 / 2) // 4*4=16
   square(2 + 17); //19*19=361
   square(square(15)); //15 * 15 = 225  *225=50625
   ```

2. Write a sentence in plain English describing how `square(square(15))` is
   evaluated.
   
   In square(square(15)), I first found the square of 15, which is 225. Then, I found the square of that result, which is 50625.

3. Rename `square`'s `num` parameter in your above code to `monkey`, and
   rename the uses of that parameter in the body to `monkey` as well. Will the
   function `square` still work? Why or why not?
YES  it will still work  the name of the parameter doesn't affect the function's functionality. 
...
4. What is wrong with the following definitions of `square`? Write a sentence or
   ...
   ```
   the issues with these erroneous versions are using the wrong variable names in the function body and using invalid values as parameter names.
